...
export class AboutPageComponent implements OnInit {
  ...
  private pendingStatsInterval = setInterval(() => {
    this.waitForStats();
  }, 250);
  ...
  getStatTint(i) {
    ...
    let rt = base[0] + ((0.5 * 1) / (len - 1 - i + 1)) * (255 - base[0]);
    let gt = base[1] + ((0.5 * 1) / (len - 1 - i + 1)) * (255 - base[1]);
    let bt = base[2] + ((0.5 * 1) / (len - 1 - i + 1)) * (255 - base[2]);
  ...
  handleStats() {
    ...
    this.totalTime =
      this.stats["human_readable_total_including_other_language"];
    ...
    this.iterAmt =
      this.languagesUsed.length >= this.topLangLength
        ? this.topLangLength
        : this.languagesUsed.length;
  ...
  ngOnInit(): void {
    this.statsService.getMyStats().subscribe((data) => {
      ...
      if (this.stats === undefined) {
        this.totalTime = "waka-error";
      } else {
        if (
          this.stats.status !== "pending_update" ||
          this.stats.status !== "updating"
        ) {
          this.handleStats();
        } else if (
          this.stats.status === "pending_update" ||
          ...
          this.stats.status === undefined
        ) {
          this.totalTime = "waka-error";
        } else {
          this.totalTime = "0 secs";
        }
      }
    });
  }
